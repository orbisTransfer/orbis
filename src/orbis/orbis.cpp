...
#include "random.h"
  ...
  void orbis_ops::generate_keys(PublicKey &pub, SecretKey &sec) {
  ...
  bool orbis_ops::check_key(const PublicKey &key) {
    ge_p3 point;
    return ge_frombytes_vartime(&point, reinterpret_cast<const unsigned char*>(&key)) == 0;
  }

  bool orbis_ops::secret_key_to_public_key(const SecretKey &sec, PublicKey &pub) {
    ge_p3 point;
  ...
  bool orbis_ops::generate_key_derivation(const PublicKey &key1, const SecretKey &key2, KeyDerivation &derivation) {
    ge_p3 point;
    ge_p2 point2;
    ge_p1p1 point3;
  ...
  bool orbis_ops::derive_public_key(const KeyDerivation &derivation, size_t output_index,
  ...
  bool orbis_ops::underive_public_key_and_get_scalar(const KeyDerivation &derivation, size_t output_index,
  ...
  void orbis_ops::derive_secret_key(const KeyDerivation &derivation, size_t output_index,
  ...
  bool orbis_ops::underive_public_key(const KeyDerivation &derivation, size_t output_index,
  ...
  void orbis_ops::generate_signature(const Hash &prefix_hash, const PublicKey &pub, const SecretKey &sec, Signature &sig) {
    lock_guard<mutex> lock(random_lock);
    ge_p3 tmp3;
  ...
  bool orbis_ops::check_signature(const Hash &prefix_hash, const PublicKey &pub, const Signature &sig) {
    ge_p2 tmp2;
    ge_p3 tmp3;
  ...
  void orbis_ops::hash_data_to_ec(const uint8_t* data, std::size_t len, PublicKey& key) {
    Hash h;
    ge_p2 point;
    ge_p1p1 point2;
  ...
  void orbis_ops::generate_key_image(const PublicKey &pub, const SecretKey &sec, KeyImage &image) {
    ge_p3 point;
    ge_p2 point2;
    assert(sc_check(reinterpret_cast<const unsigned char*>(&sec)) == 0);
  ...
  void orbis_ops::generate_incomplete_key_image(const PublicKey &pub, EllipticCurvePoint &incomplete_key_image) {
    ge_p3 point;
    hash_to_ec(pub, point);
...
#pragma warning(disable: 4200)
  ...
  void orbis_ops::generate_ring_signature(const Hash &prefix_hash, const KeyImage &image,
  ...
  bool orbis_ops::check_ring_signature(const Hash &prefix_hash, const KeyImage &image,
    const PublicKey *const *pubs, size_t pubs_count,
    const Signature *sig) {
    size_t i;
    ge_p3 image_unp;
