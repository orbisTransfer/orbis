...
std::vector<Crypto::Hash> getBlockHashes(const Orbis::CompleteBlock* blocks, size_t count) {
...
TransfersConsumer::TransfersConsumer(const Orbis::Currency& currency, INode& node, const SecretKey& viewSecret) :
...
std::error_code TransfersConsumer::onPoolUpdated(const std::vector<std::unique_ptr<ITransactionReader>>& addedTransactions, const std::vector<Hash>& deletedTransactions) {
  ...
  std::error_code processingError;
  for (auto& orbisTransaction : addedTransactions) {
    m_poolTxs.emplace(orbisTransaction->getTransactionHash());
    processingError = processTransaction(unconfirmedBlockInfo, *orbisTransaction.get());
    if (processingError) {
