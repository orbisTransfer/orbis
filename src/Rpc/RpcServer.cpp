...
RpcServer::RpcServer(System::Dispatcher& dispatcher, Logging::ILogger& log, core& c, NodeServer& p2p, const IOrbisProtocolQuery& protocolQuery) :
...
bool RpcServer::onGetPoolChanges(const COMMAND_RPC_GET_POOL_CHANGES::request& req, COMMAND_RPC_GET_POOL_CHANGES::response& rsp) {
  ...
  std::vector<Orbis::Transaction> addedTransactions;
...
bool RpcServer::on_getblocktemplate(const COMMAND_RPC_GETBLOCKTEMPLATE::request& req, COMMAND_RPC_GETBLOCKTEMPLATE::response& res) {
  ...
  Orbis::BinaryArray blob_reserve;
  ...
  PublicKey tx_pub_key = Orbis::getTransactionPublicKeyFromExtra(b.baseTransaction.extra);
