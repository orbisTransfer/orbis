...
core::core(const Currency& currency, i_orbis_protocol* pprotocol, Logging::ILogger& logger) :
m_currency(currency),
logger(logger, "core"),
...
void core::set_orbis_protocol(i_orbis_protocol* pprotocol) {
  if (pprotocol)
...
bool core::handle_incoming_tx(const BinaryArray& tx_blob, tx_verification_context& tvc, bool keeped_by_block) { //Deprecated. Should be removed with OrbisProtocolHandler.
...
bool core::handle_get_objects(NOTIFY_REQUEST_GET_OBJECTS::request& arg, NOTIFY_RESPONSE_GET_OBJECTS::request& rsp) { //Deprecated. Should be removed with OrbisProtocolHandler.
