...
bool Currency::parseAccountAddressString(const std::string& str, AccountPublicAddress& addr) const {
  uint64_t prefix;
  if (!Orbis::parseAccountAddressString(prefix, addr, str)) {
    return false;
  }
...
CurrencyBuilder::CurrencyBuilder(Logging::ILogger& log) : m_currency(log) {
  maxBlockNumber(parameters::ORBIS_MAX_BLOCK_NUMBER);
  maxBlockBlobSize(parameters::ORBIS_MAX_BLOCK_BLOB_SIZE);
  maxTxSize(parameters::ORBIS_MAX_TX_SIZE);
  publicAddressBase58Prefix(parameters::ORBIS_PUBLIC_ADDRESS_BASE58_PREFIX);
  minedMoneyUnlockWindow(parameters::ORBIS_MINED_MONEY_UNLOCK_WINDOW);

  timestampCheckWindow(parameters::BLOCKCHAIN_TIMESTAMP_CHECK_WINDOW);
  blockFutureTimeLimit(parameters::ORBIS_BLOCK_FUTURE_TIME_LIMIT);

  moneySupply(parameters::MONEY_SUPPLY);
  emissionSpeedFactor(parameters::EMISSION_SPEED_FACTOR);

  rewardBlocksWindow(parameters::ORBIS_REWARD_BLOCKS_WINDOW);
  blockGrantedFullRewardZone(parameters::ORBIS_BLOCK_GRANTED_FULL_REWARD_ZONE);
  minerTxBlobReservedSize(parameters::ORBIS_COINBASE_BLOB_RESERVED_SIZE);

  numberOfDecimalPlaces(parameters::ORBIS_DISPLAY_DECIMAL_POINT);
  ...
  lockedTxAllowedDeltaSeconds(parameters::ORBIS_LOCKED_TX_ALLOWED_DELTA_SECONDS);
  lockedTxAllowedDeltaBlocks(parameters::ORBIS_LOCKED_TX_ALLOWED_DELTA_BLOCKS);

  mempoolTxLiveTime(parameters::ORBIS_MEMPOOL_TX_LIVETIME);
  mempoolTxFromAltBlockLiveTime(parameters::ORBIS_MEMPOOL_TX_FROM_ALT_BLOCK_LIVETIME);
  numberOfPeriodsToForgetTxDeletedFromPool(parameters::ORBIS_NUMBER_OF_PERIODS_TO_FORGET_TX_DELETED_FROM_POOL);

  fusionTxMaxSize(parameters::FUSION_TX_MAX_SIZE);
  fusionTxMinInputCount(parameters::FUSION_TX_MIN_INPUT_COUNT);
  fusionTxMinInOutCountRatio(parameters::FUSION_TX_MIN_IN_OUT_COUNT_RATIO);

  blocksFileName(parameters::ORBIS_BLOCKS_FILENAME);
  blocksCacheFileName(parameters::ORBIS_BLOCKSCACHE_FILENAME);
  blockIndexesFileName(parameters::ORBIS_BLOCKINDEXES_FILENAME);
  txPoolFileName(parameters::ORBIS_POOLDATA_FILENAME);
  blockchinIndicesFileName(parameters::ORBIS_BLOCKCHAIN_INDICES_FILENAME);
...
Transaction CurrencyBuilder::generateGenesisTransaction() {
  ...
  Orbis::AccountPublicAddress ac = boost::value_initialized<Orbis::AccountPublicAddress>();
