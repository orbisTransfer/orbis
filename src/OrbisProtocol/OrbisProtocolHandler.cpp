...
#include "OrbisProtocolHandler.h"
...
bool post_notify(IP2pEndpoint& p2p, typename t_parametr::request& arg, const OrbisConnectionContext& context) {
...
OrbisProtocolHandler::OrbisProtocolHandler(const Currency& currency, System::Dispatcher& dispatcher, ICore& rcore, IP2pEndpoint* p_net_layout, Logging::ILogger& log) :
...
size_t OrbisProtocolHandler::getPeerCount() const {
  return m_peersCount;
}

void OrbisProtocolHandler::set_p2p_endpoint(IP2pEndpoint* p2p) {
  if (p2p)
    m_p2p = p2p;
  else
    m_p2p = &m_p2p_stub;
...
void OrbisProtocolHandler::onConnectionOpened(OrbisConnectionContext& context) {
}

void OrbisProtocolHandler::onConnectionClosed(OrbisConnectionContext& context) {
  bool updated = false;
  {
  ...
  if (updated) {
    ...
    m_observerManager.notify(&IOrbisProtocolObserver::lastKnownBlockHeightUpdated, m_observedHeight);
  }

  if (context.m_state != OrbisConnectionContext::state_befor_handshake) {
    m_peersCount--;
    m_observerManager.notify(&IOrbisProtocolObserver::peerCountUpdated, m_peersCount.load());
  }
}

void OrbisProtocolHandler::stop() {
  m_stop = true;
}

bool OrbisProtocolHandler::start_sync(OrbisConnectionContext& context) {
  logger(Logging::TRACE) << context << "Starting synchronization";

  if (context.m_state == OrbisConnectionContext::state_synchronizing) {
...
bool OrbisProtocolHandler::get_stat_info(core_stat_info& stat_inf) {
  return m_core.get_stat_info(stat_inf);
}

void OrbisProtocolHandler::log_connections() {
  std::stringstream ss;
  ...
  m_p2p->for_each_connection([&](const OrbisConnectionContext& cntxt, PeerIdType peer_id) {
...
uint32_t OrbisProtocolHandler::get_current_blockchain_height() {
  uint32_t height;
  Crypto::Hash blockId;
...
bool OrbisProtocolHandler::process_payload_sync_data(const CORE_SYNC_DATA& hshd, OrbisConnectionContext& context, bool is_inital) {
  if (context.m_state == OrbisConnectionContext::state_befor_handshake && !is_inital)
    return true;
  ...
  if (context.m_state == OrbisConnectionContext::state_synchronizing) {
    ...
    if (is_inital) {
      ...
      context.m_state = OrbisConnectionContext::state_pool_sync_required;
    } else {
      context.m_state = OrbisConnectionContext::state_normal;
    }
  } else {
    ...
    context.m_state = OrbisConnectionContext::state_sync_required;
  }
...
bool OrbisProtocolHandler::get_payload_sync_data(CORE_SYNC_DATA& hshd) {
...
int notifyAdaptor(const BinaryArray& reqBuf, OrbisConnectionContext& ctx, Handler handler) {
...
int OrbisProtocolHandler::handleCommand(bool is_notify, int command, const BinaryArray& in, BinaryArray& out, OrbisConnectionContext& ctx, bool& handled) {
  int ret = 0;
  handled = true;

  switch (command) {
    HANDLE_NOTIFY(NOTIFY_NEW_BLOCK, &OrbisProtocolHandler::handle_notify_new_block)
    HANDLE_NOTIFY(NOTIFY_NEW_TRANSACTIONS, &OrbisProtocolHandler::handle_notify_new_transactions)
    HANDLE_NOTIFY(NOTIFY_REQUEST_GET_OBJECTS, &OrbisProtocolHandler::handle_request_get_objects)
    HANDLE_NOTIFY(NOTIFY_RESPONSE_GET_OBJECTS, &OrbisProtocolHandler::handle_response_get_objects)
    HANDLE_NOTIFY(NOTIFY_REQUEST_CHAIN, &OrbisProtocolHandler::handle_request_chain)
    HANDLE_NOTIFY(NOTIFY_RESPONSE_CHAIN_ENTRY, &OrbisProtocolHandler::handle_response_chain_entry)
    HANDLE_NOTIFY(NOTIFY_REQUEST_TX_POOL, &OrbisProtocolHandler::handleRequestTxPool)
...
int OrbisProtocolHandler::handle_notify_new_block(int command, NOTIFY_NEW_BLOCK::request& arg, OrbisConnectionContext& context) {
  ...
  if (context.m_state != OrbisConnectionContext::state_normal) {
    return 1;
  }

  for (auto tx_blob_it = arg.b.txs.begin(); tx_blob_it != arg.b.txs.end(); tx_blob_it++) {
    Orbis::tx_verification_context tvc = boost::value_initialized<decltype(tvc)>();
    ...
    if (tvc.m_verifivation_failed) {
      ...
      context.m_state = OrbisConnectionContext::state_shutdown;
      return 1;
    }
  }
  ...
  if (bvc.m_added_to_main_chain) {
    ...
    context.m_state = OrbisConnectionContext::state_synchronizing;
    NOTIFY_REQUEST_CHAIN::request r = boost::value_initialized<NOTIFY_REQUEST_CHAIN::request>();
    r.block_ids = m_core.buildSparseChain();
    logger(Logging::TRACE) << context << "-->>NOTIFY_REQUEST_CHAIN: m_block_ids.size()=" << r.block_ids.size();
    post_notify<NOTIFY_REQUEST_CHAIN>(*m_p2p, r, context);
...
int OrbisProtocolHandler::handle_notify_new_transactions(int command, NOTIFY_NEW_TRANSACTIONS::request& arg, OrbisConnectionContext& context) {
...
int OrbisProtocolHandler::handle_request_get_objects(int command, NOTIFY_REQUEST_GET_OBJECTS::request& arg, OrbisConnectionContext& context) {
...
int OrbisProtocolHandler::handle_response_get_objects(int command, NOTIFY_RESPONSE_GET_OBJECTS::request& arg, OrbisConnectionContext& context) {
  ...
  for (const block_complete_entry& block_entry : arg.blocks) {
    ...
    if (count == 2) {
      if (m_core.have_block(get_block_hash(b))) {
        context.m_state = OrbisConnectionContext::state_idle;
  ...
  if (!m_stop && context.m_state == OrbisConnectionContext::state_synchronizing) {
...
int OrbisProtocolHandler::processObjects(OrbisConnectionContext& context, const std::vector<block_complete_entry>& blocks) {
...
bool OrbisProtocolHandler::on_idle() {
  return m_core.on_idle();
}

int OrbisProtocolHandler::handle_request_chain(int command, NOTIFY_REQUEST_CHAIN::request& arg, OrbisConnectionContext& context) {
...
bool OrbisProtocolHandler::request_missing_objects(OrbisConnectionContext& context, bool check_having_blocks) {
...
bool OrbisProtocolHandler::on_connection_synchronized() {
  ...
  if (m_synchronized.compare_exchange_strong(val_expected, true)) {
    ...
    m_observerManager.notify(&IOrbisProtocolObserver::blockchainSynchronized, height);
  }
  return true;
}

int OrbisProtocolHandler::handle_response_chain_entry(int command, NOTIFY_RESPONSE_CHAIN_ENTRY::request& arg, OrbisConnectionContext& context) {
...
int OrbisProtocolHandler::handleRequestTxPool(int command, NOTIFY_REQUEST_TX_POOL::request& arg,
                                                     OrbisConnectionContext& context) {
...
void OrbisProtocolHandler::relay_block(NOTIFY_NEW_BLOCK::request& arg) {
  auto buf = LevinProtocol::encode(arg);
  m_p2p->externalRelayNotifyToAll(NOTIFY_NEW_BLOCK::ID, buf);
}

void OrbisProtocolHandler::relay_transactions(NOTIFY_NEW_TRANSACTIONS::request& arg) {
  auto buf = LevinProtocol::encode(arg);
  m_p2p->externalRelayNotifyToAll(NOTIFY_NEW_TRANSACTIONS::ID, buf);
}

void OrbisProtocolHandler::requestMissingPoolTransactions(const OrbisConnectionContext& context) {
...
void OrbisProtocolHandler::updateObservedHeight(uint32_t peerHeight, const OrbisConnectionContext& context) {
  bool updated = false;
  {
    std::lock_guard<std::mutex> lock(m_observedHeightMutex);
...
void OrbisProtocolHandler::recalculateMaxObservedHeight(const OrbisConnectionContext& context) {
  //should be locked outside
  uint32_t peerHeight = 0;
  m_p2p->for_each_connection([&peerHeight, &context](const OrbisConnectionContext& ctx, PeerIdType peerId) {
...
uint32_t OrbisProtocolHandler::getObservedHeight() const {
  std::lock_guard<std::mutex> lock(m_observedHeightMutex);
  return m_observedHeight;
};

bool OrbisProtocolHandler::addObserver(IOrbisProtocolObserver* observer) {
  return m_observerManager.add(observer);
}

bool OrbisProtocolHandler::removeObserver(IOrbisProtocolObserver* observer) {
