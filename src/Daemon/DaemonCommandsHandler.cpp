...
namespace {
  ...
  static bool print_as_json(const T& obj) {
    std::cout << Orbis::storeToJson(obj) << ENDL;
    return true;
  }
}


DaemonCommandsHandler::DaemonCommandsHandler(Orbis::core& core, Orbis::NodeServer& srv, Logging::LoggerManager& log) :
...
{
  std::stringstream ss;
  ss << Orbis::ORBIS_NAME << " v" << PROJECT_VERSION_LONG << ENDL;
  ss << "Commands: " << ENDL;
...
bool DaemonCommandsHandler::print_tx(const std::vector<std::string>& args)
{
  ...
  std::list<Orbis::Transaction> txs;
  std::list<Crypto::Hash> missed_ids;
...
bool DaemonCommandsHandler::start_mining(const std::vector<std::string> &args) {
  ...
  Orbis::AccountPublicAddress adr;
