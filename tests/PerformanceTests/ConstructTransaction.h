...
public:
  ...
  bool test()
  {
    return Orbis::constructTransaction(this->m_miners[this->real_source_idx].getAccountKeys(), this->m_sources, m_destinations, std::vector<uint8_t>(), m_tx, 0, this->m_logger);
  }

private:
  Orbis::AccountBase m_alice;
  std::vector<Orbis::TransactionDestinationEntry> m_destinations;
  Orbis::Transaction m_tx;
};
