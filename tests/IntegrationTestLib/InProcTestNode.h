...
public:
  InProcTestNode(const TestNodeConfiguration& cfg, const Orbis::Currency& currency);
  ~InProcTestNode();
  ...
  virtual bool getBlockTemplate(const std::string &minerAddress, Orbis::Block &blockTemplate, uint64_t &difficulty) override;
  virtual bool submitBlock(const std::string& block) override;
  virtual bool getTailBlockId(Crypto::Hash &tailBlockId) override;
  virtual bool makeINode(std::unique_ptr<Orbis::INode>& node) override;
...
private:
  ...
  std::unique_ptr<Orbis::core> core;
  std::unique_ptr<Orbis::OrbisProtocolHandler> protocol;
  std::unique_ptr<Orbis::NodeServer> p2pNode;
