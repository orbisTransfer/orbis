...
#ifdef _WIN32
const std::string DAEMON_FILENAME = std::string(ORBIS_NAME) + "d.exe";
#else
const std::string DAEMON_FILENAME = std::string(ORBIS_NAME) + "d";
#endif
...
void BaseFunctionalTests::startNode(size_t index) {
  ...
  config
    ...
    << "log-file=test_" << ORBIS_NAME << "d_" << index << ".log" << std::endl;
...
#if defined WIN32
  std::string commandLine = "start /MIN \"" + std::string(ORBIS_NAME) + "d" + std::to_string(index) + "\" \"" + daemonPath.string() +
  ...
  if (pid == 0) {
    ...
    if (execl(pathToDaemon.c_str(), (std::string(ORBIS_NAME) + "d").c_str(), "--testnet", dataDir.c_str(), "--config-file=daemon.conf", NULL) == -1) {
...
namespace {
  class WaitForCoinBaseObserver : public Orbis::IWalletLegacyObserver {
    Semaphore& m_gotReward;
    Orbis::IWalletLegacy& m_wallet;
  public:
    WaitForCoinBaseObserver(Semaphore& gotReward, Orbis::IWalletLegacy& wallet) : m_gotReward(gotReward), m_wallet(wallet) { }
    virtual void externalTransactionCreated(Orbis::TransactionId transactionId) override {
      Orbis::WalletLegacyTransaction trInfo;
...
bool BaseFunctionalTests::mineBlocks(TestNode& node, const Orbis::AccountPublicAddress& address, size_t blockCount) {
...
bool BaseFunctionalTests::prepareAndSubmitBlock(TestNode& node, Orbis::Block&& blockTemplate) {
  blockTemplate.timestamp = m_nextTimestamp;
  m_nextTimestamp += 2 * m_currency.difficultyTarget();

  BinaryArray blockBlob = Orbis::toBinaryArray(blockTemplate);
  return node.submitBlock(::Common::toHex(blockBlob.data(), blockBlob.size()));
}

bool BaseFunctionalTests::mineBlock(std::unique_ptr<Orbis::IWalletLegacy> &wallet) {
...
bool BaseFunctionalTests::makeWallet(std::unique_ptr<Orbis::IWalletLegacy> & wallet, std::unique_ptr<Orbis::INode>& node, const std::string& password) {
  if (!node) return false;
  wallet = std::unique_ptr<Orbis::IWalletLegacy>(new Orbis::WalletLegacy(m_currency, *node));
...
namespace {
  struct PeerCountWaiter : Orbis::INodeObserver {
...
bool BaseFunctionalTests::waitForPeerCount(Orbis::INode& node, size_t expectedPeerCount) {
...
bool BaseFunctionalTests::waitForPoolSize(size_t nodeIndex, Orbis::INode& node, size_t expectedPoolSize,
...
bool BaseFunctionalTests::getNodeTransactionPool(size_t nodeIndex, Orbis::INode& node,
  std::vector<std::unique_ptr<Orbis::ITransactionReader>>& txPool) {
