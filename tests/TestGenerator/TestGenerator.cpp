...
uint64_t test_generator::getAlreadyGeneratedCoins(const Orbis::Block& blk) const {
  Crypto::Hash blkHash;
  get_block_hash(blk, blkHash);
  return getAlreadyGeneratedCoins(blkHash);
}

void test_generator::addBlock(const Orbis::Block& blk, size_t tsxSize, uint64_t fee,
...
bool test_generator::constructBlock(Orbis::Block& blk, uint32_t height, const Crypto::Hash& previousBlockHash,
                                    const Orbis::AccountBase& minerAcc, uint64_t timestamp, uint64_t alreadyGeneratedCoins,
                                    std::vector<size_t>& blockSizes, const std::list<Orbis::Transaction>& txList) {
...
bool test_generator::constructBlock(Orbis::Block& blk, const Orbis::AccountBase& minerAcc, uint64_t timestamp) {
  std::vector<size_t> blockSizes;
  std::list<Orbis::Transaction> txList;
  return constructBlock(blk, 0, NULL_HASH, minerAcc, timestamp, 0, blockSizes, txList);
}

bool test_generator::constructBlock(Orbis::Block& blk, const Orbis::Block& blkPrev,
                                    const Orbis::AccountBase& minerAcc,
                                    const std::list<Orbis::Transaction>& txList/* = std::list<Orbis::Transaction>()*/) {
...
bool test_generator::constructBlockManuallyTx(Orbis::Block& blk, const Orbis::Block& prevBlock,
                                              const Orbis::AccountBase& minerAcc,
...
bool test_generator::constructMaxSizeBlock(Orbis::Block& blk, const Orbis::Block& blkPrev,
                                           const Orbis::AccountBase& minerAccount,
...
void fillNonce(Orbis::Block& blk, const difficulty_type& diffic) {
  blk.nonce = 0;
  Crypto::cn_context context;
...
bool constructMinerTxManually(const Orbis::Currency& currency, uint32_t height, uint64_t alreadyGeneratedCoins,
...
bool constructMinerTxBySize(const Orbis::Currency& currency, Orbis::Transaction& baseTransaction, uint32_t height,
                            uint64_t alreadyGeneratedCoins, const Orbis::AccountPublicAddress& minerAddress,
  ...
  if (!currency.constructMinerTx(height, Common::medianValue(blockSizes), alreadyGeneratedCoins, targetBlockSize,
      fee, minerAddress, baseTransaction, Orbis::BinaryArray(), 1)) {
    return false;
  }
