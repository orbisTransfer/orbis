...
namespace {
  bool lift_up_difficulty(const Orbis::Currency& currency, std::vector<test_event_entry>& events,
                          std::vector<uint64_t>& timestamps,
                          std::vector<Orbis::difficulty_type>& cummulative_difficulties, test_generator& generator,
                          size_t new_block_count, const Orbis::Block blk_last,
                          const Orbis::AccountBase& miner_account, uint8_t block_major_version) {
    Orbis::difficulty_type commulative_diffic = cummulative_difficulties.empty() ? 0 : cummulative_difficulties.back();
    Orbis::Block blk_prev = blk_last;
    for (size_t i = 0; i < new_block_count; ++i) {
      Orbis::Block blk_next;
      Orbis::difficulty_type diffic = currency.nextDifficulty(timestamps, cummulative_difficulties);
  ...
  void clearTransaction(Orbis::Transaction& tx) {
    tx.version = 0;
    tx.unlockTime = 0;
    tx.inputs.clear();
    tx.outputs.clear();
...
bool gen_block_invalid_prev_id::check_block_verification_context(const Orbis::block_verification_context& bvc, size_t event_idx, const Orbis::Block& /*blk*/)
{
  if (1 == event_idx)
...
bool gen_block_invalid_binary_format::check_block_verification_context(const Orbis::block_verification_context& bvc,
                                                                       size_t event_idx, const Orbis::Block& blk)
{
...
bool gen_block_invalid_binary_format::corrupt_blocks_boundary(Orbis::core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
  m_corrupt_blocks_begin_idx = ev_index + 1;
  return true;
}

bool gen_block_invalid_binary_format::check_all_blocks_purged(Orbis::core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
