...
public:
  ...
  void generate(const AccountPublicAddress& address, Transaction& tx, uint64_t unlockTime = 0)
  {
    ...
    Orbis::decompose_amount_into_digits(this->m_source_amount, 0,
      [&](uint64_t chunk) { destinations.push_back(Orbis::TransactionDestinationEntry(chunk, address)); },
      [&](uint64_t a_dust) { destinations.push_back(Orbis::TransactionDestinationEntry(a_dust, address)); });

    Orbis::constructTransaction(this->m_miners[this->real_source_idx].getAccountKeys(), this->m_sources, destinations, std::vector<uint8_t>(), tx, unlockTime, m_logger);
  }
...
TestBlockchainGenerator::TestBlockchainGenerator(const Orbis::Currency& currency) :
  m_currency(currency),
  generator(currency)
{
...
std::vector<Orbis::Block>& TestBlockchainGenerator::getBlockchain()
{
  std::unique_lock<std::mutex> lock(m_mutex);
  return m_blockchain;
}

std::vector<Orbis::Block> TestBlockchainGenerator::getBlockchainCopy() {
  std::unique_lock<std::mutex> lock(m_mutex);

  std::vector<Orbis::Block> blockchain(m_blockchain);
  return blockchain;
}

bool TestBlockchainGenerator::getTransactionByHash(const Crypto::Hash& hash, Orbis::Transaction& tx, bool checkTxPool)
{
  std::unique_lock<std::mutex> lock(m_mutex);
...
const Orbis::AccountBase& TestBlockchainGenerator::getMinerAccount() const {
  std::unique_lock<std::mutex> lock(m_mutex);
  return miner_acc;
}
...
void TestBlockchainGenerator::addGenesisBlock() {
  ...
  m_timestampIndex.add(m_currency.genesisBlock().timestamp, Orbis::get_block_hash(m_currency.genesisBlock()));
...
void TestBlockchainGenerator::addMiningBlock() {
  ...
  Orbis::Block& prev_block = m_blockchain.back();
  ...
  m_timestampIndex.add(block.timestamp, Orbis::get_block_hash(block));
...
void TestBlockchainGenerator::addTxToBlockchain(const Orbis::Transaction& transaction)
{
  std::unique_lock<std::mutex> lock(m_mutex);
  addToBlockchain(transaction);
}

bool TestBlockchainGenerator::getBlockRewardForAddress(const Orbis::AccountPublicAddress& address)
{
  std::unique_lock<std::mutex> lock(m_mutex);
...
bool TestBlockchainGenerator::generateTransactionsInOneBlock(const Orbis::AccountPublicAddress& address, size_t n) {
  std::unique_lock<std::mutex> lock(m_mutex);
...
bool TestBlockchainGenerator::getSingleOutputTransaction(const Orbis::AccountPublicAddress& address, uint64_t amount) {
  std::unique_lock<std::mutex> lock(m_mutex);
...
void TestBlockchainGenerator::addToBlockchain(const Orbis::Transaction& tx) {
  addToBlockchain(std::vector<Orbis::Transaction> {tx});
}

void TestBlockchainGenerator::addToBlockchain(const std::vector<Orbis::Transaction>& txs) {
  addToBlockchain(txs, miner_acc);
}

void TestBlockchainGenerator::addToBlockchain(const std::vector<Orbis::Transaction>& txs, const Orbis::AccountBase& minerAddress) {
  std::list<Orbis::Transaction> txsToBlock;
...
void TestBlockchainGenerator::getPoolSymmetricDifference(std::vector<Crypto::Hash>&& known_pool_tx_ids, Crypto::Hash known_block_id, bool& is_bc_actual,
  std::vector<Orbis::Transaction>& new_txs, std::vector<Crypto::Hash>& deleted_tx_ids)
{
  std::unique_lock<std::mutex> lock(m_mutex);

  if (known_block_id != Orbis::get_block_hash(m_blockchain.back())) {
    is_bc_actual = false;
    return;
  }
...
void TestBlockchainGenerator::putTxToPool(const Orbis::Transaction& tx) {
  std::unique_lock<std::mutex> lock(m_mutex);

  Crypto::Hash txHash = Orbis::getObjectHash(tx);
  m_txPool[txHash] = tx;
}

void TestBlockchainGenerator::putTxPoolToBlockchain() {
  std::unique_lock<std::mutex> lock(m_mutex);
  std::vector<Orbis::Transaction> txs;
...
void TestBlockchainGenerator::setMinerAccount(const Orbis::AccountBase& account) {
  miner_acc = account;
}
...
bool TestBlockchainGenerator::getPoolTransactionIdsByTimestamp(uint64_t timestampBegin, uint64_t timestampEnd, uint32_t transactionsNumberLimit, std::vector<Crypto::Hash>& hashes, uint64_t& transactionsNumberWithinTimestamps) {
  ...
  for (auto i : m_txPool) {
    ...
    hashes.push_back(Orbis::getObjectHash(i.second));
    ++c;
  }
  return true;
}
...
void TestBlockchainGenerator::addTx(const Orbis::Transaction& tx) {
...
bool TestBlockchainGenerator::generateFromBaseTx(const Orbis::AccountBase& address) {
  std::unique_lock<std::mutex> lock(m_mutex);
