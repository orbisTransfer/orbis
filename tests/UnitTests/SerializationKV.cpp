...
TEST(KVSerialize, Simple) {
  ...
  std::string buf = Orbis::storeToBinaryKeyValue(testData1);
  ASSERT_TRUE(Orbis::loadFromBinaryKeyValue(testData2, buf));
...
TEST(KVSerialize, BigCollection) {
  ...
  std::string buf = Orbis::storeToBinaryKeyValue(ts1);
  ASSERT_TRUE(Orbis::loadFromBinaryKeyValue(ts2, buf));
  EXPECT_EQ(ts1, ts2);
}
