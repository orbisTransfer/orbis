...
#include "OrbisCore/TransactionPool.h"
...
class TransactionValidator : public Orbis::ITransactionValidator {
  virtual bool checkTransactionInputs(const Orbis::Transaction& tx, BlockInfo& maxUsedBlock) override {
    return true;
  }

  virtual bool checkTransactionInputs(const Orbis::Transaction& tx, BlockInfo& maxUsedBlock, BlockInfo& lastFailed) override {
    return true;
  }

  virtual bool haveSpentKeyImages(const Orbis::Transaction& tx) override {
    return false;
  }
...
public:
  ...
  TestTransactionGenerator(const Orbis::Currency& currency, size_t ringSize) :
    m_currency(currency),
    m_ringSize(ringSize),
    m_miners(ringSize), 
...
namespace
{
  ...
  void GenerateTransaction(const Orbis::Currency& currency, Transaction& tx, uint64_t fee, size_t outputs) {
  ...
  public:
    ...
    TestPool(const Orbis::Currency& currency, Logging::ILogger& logger) :
...
public:
  TxPool_FillBlockTemplate() :
    tx_pool() {
    currency = Orbis::CurrencyBuilder(logger).fusionTxMaxSize(TEST_FUSION_TX_MAX_SIZE).blockGrantedFullRewardZone(TEST_MEDIAN_SIZE).currency();
  }
