...
#include "ICoreStub.h"
#include "IOrbisProtocolQueryStub.h"
...
namespace {
Orbis::Transaction createTx(Orbis::ITransactionReader& tx) {
  Orbis::Transaction outTx;
...
protected:
  ...
  ICoreStub coreStub;
  IOrbisProtocolQueryStub protocolQueryStub;
  Orbis::InProcessNode node;
...
TEST_F(InProcessNodeTests, initOk) {
  Orbis::InProcessNode newNode(coreStub, protocolQueryStub);
  CallbackStatus status;
...
TEST_F(InProcessNodeTests, getRandomOutsByAmountsSuccess) {
  ...
  Orbis::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response expectedResp;
  Orbis::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_outs_for_amount out;
  out.amount = 10;
  out.outs.push_back({ 11, ignoredPublicKey });
  expectedResp.outs.push_back(out);
  coreStub.set_random_outs(expectedResp, true);

  std::vector<Orbis::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_outs_for_amount> outs;
...
TEST_F(InProcessNodeTests, getNewBlocksUninitialized) {
  ...
  std::vector<Orbis::block_complete_entry> newBlocks;
...
TEST_F(InProcessNodeTests, relayTransactionUninitialized) {
  ...
  CallbackStatus status;
  newNode.relayTransaction(Orbis::Transaction(), [&] (std::error_code ec) { status.setStatus(ec); });
  ASSERT_TRUE(status.wait());
  ASSERT_NE(std::error_code(), status.getStatus());
}
...
TEST_F(InProcessNodeTests, getBlocksByHeightEmpty) {
  ...
  std::vector<std::vector<Orbis::BlockDetails>> blocks;
...
TEST_F(InProcessNodeTests, getBlocksByHeightMany) {
  ...
  std::vector<uint32_t> blockHeights;
  std::vector<std::vector<Orbis::BlockDetails>> actualBlocks;

  std::vector<Orbis::Block> expectedBlocks;
  ...
  for (auto iter = generator.getBlockchain().begin() + 1; iter != generator.getBlockchain().end(); iter++) {
    ...
    blockHeights.push_back(std::move(boost::get<Orbis::BaseInput>(iter->baseTransaction.inputs.front()).blockIndex));
  ...
  for (const boost::tuple<boost::tuple<size_t, Orbis::Block>, std::vector<Orbis::BlockDetails>>& sameHeight : range) {
    EXPECT_EQ(sameHeight.get<1>().size(), 1);
    for (const Orbis::BlockDetails& block : sameHeight.get<1>()) {
      EXPECT_EQ(block.height, sameHeight.get<0>().get<0>());
      Crypto::Hash expectedCryptoHash = Orbis::get_block_hash(sameHeight.get<0>().get<1>());
...
TEST_F(InProcessNodeTests, getBlocksByHeightFail) {
  ...
  for (const Orbis::Block& block : generator.getBlockchain()) {
...
TEST_F(InProcessNodeTests, getBlocksByHashEmpty) {
  ...
  std::vector<Orbis::BlockDetails> blocks;
...
TEST_F(InProcessNodeTests, getBlocksByHashMany) {
  ...
  std::vector<Crypto::Hash> blockHashes;
  std::vector<Orbis::BlockDetails> actualBlocks;
  ...
  for (auto iter = generator.getBlockchain().begin() + 1; iter != generator.getBlockchain().end(); iter++) {
    expectedBlocks.push_back(*iter);
    blockHashes.push_back(Orbis::get_block_hash(*iter));
    coreStub.addBlock(*iter);
  }
  ...
  auto range = boost::combine(range1, actualBlocks);
  for (const boost::tuple<boost::tuple<Crypto::Hash, Orbis::Block>, Orbis::BlockDetails>& sameHeight : range) {
    Crypto::Hash expectedCryptoHash = Orbis::get_block_hash(sameHeight.get<0>().get<1>());
...
TEST_F(InProcessNodeTests, getTxEmpty) {
  ...
  std::vector<Orbis::TransactionDetails> transactions;
...
TEST_F(InProcessNodeTests, getTxMany) {
  ...
  std::vector<Crypto::Hash> transactionHashes;
  std::vector<Orbis::TransactionDetails> actualTransactions;

  std::vector<std::tuple<Orbis::Transaction, Crypto::Hash, uint64_t>> expectedTransactions;

  coreStub.set_blockchain_top(0, boost::value_initialized<Crypto::Hash>());

  size_t prevBlockchainSize = generator.getBlockchain().size();
  for (size_t i = 0; i < BLOCKCHAIN_TX_NUMBER; ++i) {
    auto txptr = Orbis::createTransaction();
    auto tx = ::createTx(*txptr.get());
    transactionHashes.push_back(Orbis::getObjectHash(tx));
    ...
    expectedTransactions.push_back(std::make_tuple(tx, Orbis::get_block_hash(generator.getBlockchain().back()), boost::get<Orbis::BaseInput>(generator.getBlockchain().back().baseTransaction.inputs.front()).blockIndex));
  }
  ...
  for (const boost::tuple<boost::tuple<Crypto::Hash, Orbis::TransactionDetails>, std::tuple<Orbis::Transaction, Crypto::Hash, uint64_t>>& sameHeight : range) {
    Crypto::Hash expectedCryptoHash = Orbis::getObjectHash(std::get<0>(sameHeight.get<1>()));
    EXPECT_EQ(expectedCryptoHash, sameHeight.get<0>().get<0>());
    Hash expectedHash = reinterpret_cast<const Hash&>(expectedCryptoHash);
...
TEST_F(InProcessNodeTests, getLastLocalBlockTimestamp) {
  ...
  Orbis::InProcessNode newNode(core, protocolQueryStub);
...
TEST_F(InProcessNodeTests, getPoolDiffereceActualBC) {
  ...
  for (size_t i = 0; i < POOL_TX_NUMBER; ++i) {
    auto txptr = Orbis::createTransaction();
    auto tx = ::createTx(*txptr.get());
    transactionHashes.insert(Orbis::getObjectHash(tx));
    Orbis::tx_verification_context tvc = boost::value_initialized<tx_verification_context>();
    bool keptByBlock = false;
    coreStub.handleIncomingTransaction(tx, Orbis::getObjectHash(tx), Orbis::getObjectBinarySize(tx), tvc, keptByBlock);
  ...
  std::vector<Crypto::Hash> knownPoolTxIds;
  Crypto::Hash knownBlockId = Orbis::getObjectHash(generator.getBlockchain().back());
  bool isBcActual = false;
  std::vector<std::unique_ptr<ITransactionReader>> newTxs;
  std::vector<Crypto::Hash> deletedTxIds;
