...
class TrivialWalletObserver : public Orbis::IWalletLegacyObserver
{
public:
...
struct SaveOnInitWalletObserver: public Orbis::IWalletLegacyObserver {
  SaveOnInitWalletObserver(Orbis::WalletLegacy* wallet) : wallet(wallet) {
  };
  ...
  Orbis::WalletLegacy* wallet;
  std::stringstream stream;
};

static const uint64_t TEST_BLOCK_REWARD = 70368744177663;

Orbis::TransactionId TransferMoney(Orbis::WalletLegacy& from, Orbis::WalletLegacy& to, int64_t amount, uint64_t fee, uint64_t mixIn = 0, const std::string& extra = "") {
  Orbis::WalletLegacyTransfer transfer;
...
{
public:
  WalletLegacyApi() : m_currency(Orbis::CurrencyBuilder(m_logger).currency()), generator(m_currency) {
  }
...
protected:
  ...
  void GetOneBlockReward(Orbis::WalletLegacy& wallet);
  void GetOneBlockReward(Orbis::WalletLegacy& wallet, const Orbis::Currency& currency, TestBlockchainGenerator& blockchainGenerator);
  void GetOneBlockRewardAndUnlock(Orbis::WalletLegacy& wallet, TrivialWalletObserver& observer, INodeTrivialRefreshStub& node,
                                  const Orbis::Currency& currency, TestBlockchainGenerator& blockchainGenerator);
...
void WalletLegacyApi::prepareAliceWallet() {
  ...
  alice.reset(new Orbis::WalletLegacy(m_currency, *newNode));
  aliceNode = newNode;
...
void WalletLegacyApi::prepareBobWallet() {
  ...
  bob.reset(new Orbis::WalletLegacy(m_currency, *bobNode));
...
void WalletLegacyApi::prepareCarolWallet() {
  ...
  carol.reset(new Orbis::WalletLegacy(m_currency, *carolNode));
  carol->addObserver(carolWalletObserver.get());
}

void WalletLegacyApi::GetOneBlockReward(Orbis::WalletLegacy& wallet) {
  GetOneBlockReward(wallet, m_currency, generator);
}

void WalletLegacyApi::GetOneBlockReward(Orbis::WalletLegacy& wallet, const Orbis::Currency& currency, TestBlockchainGenerator& blockchainGenerator) {
  Orbis::AccountPublicAddress address;
...
void WalletLegacyApi::GetOneBlockRewardAndUnlock(Orbis::WalletLegacy& wallet, TrivialWalletObserver& observer, INodeTrivialRefreshStub& node,
                                                 const Orbis::Currency& currency, TestBlockchainGenerator& blockchainGenerator) {
...
void WalletLegacyApi::performTransferWithErrorTx(const std::array<int64_t, 5>& amounts, uint64_t fee) {
  std::vector<Orbis::WalletLegacyTransfer> trs;
...
TEST_F(WalletLegacyApi, TransactionsAndTransfersAfterSend) {
  ...
  Orbis::WalletLegacyTransaction tx;
...
TEST_F(WalletLegacyApi, getTransactionSuccess) {
  ...
  EXPECT_EQ(tx.firstTransferId, Orbis::WALLET_LEGACY_INVALID_TRANSFER_ID);
...
TEST_F(WalletLegacyApi, wrongPassword) {
  ...
  EXPECT_EQ(result.value(), Orbis::error::WRONG_PASSWORD);
}
...
TEST_F(WalletLegacyApi, sendMoneyToMyself) {
  ...
  Orbis::TransactionId txId = TransferMoney(*alice, *alice, 100000000, 100);
  ASSERT_NE(txId, Orbis::WALLET_LEGACY_INVALID_TRANSACTION_ID);
  ASSERT_NO_FATAL_FAILURE(WaitWalletSend(aliceWalletObserver.get()));
...
TEST_F(WalletLegacyApi, checkBalanceAfterSend) {
  ...
  Orbis::TransactionId txId = TransferMoney(*alice, *alice, sendAmount, fee);
  ASSERT_NE(txId, Orbis::WALLET_LEGACY_INVALID_TRANSACTION_ID);
  ASSERT_NO_FATAL_FAILURE(WaitWalletSend(aliceWalletObserver.get()));
...
TEST_F(WalletLegacyApi, moneyInPoolDontAffectActualBalance) {
  ...
  Orbis::TransactionId txId = TransferMoney(*alice, *bob, sendAmount, fee);
  ASSERT_NE(txId, Orbis::WALLET_LEGACY_INVALID_TRANSACTION_ID);
  ASSERT_NO_FATAL_FAILURE(WaitWalletSend(aliceWalletObserver.get()));
  generator.generateEmptyBlocks(10);
...
TEST_F(WalletLegacyApi, sendAfterFailedTransaction) {
  ...
  Orbis::TransactionId txId = TransferMoney(*alice, *alice, 100000, 100);
  ASSERT_NE(txId, Orbis::WALLET_LEGACY_INVALID_TRANSACTION_ID);
  ASSERT_NO_FATAL_FAILURE(WaitWalletSend(aliceWalletObserver.get()));
  alice->shutdown();
}
...
TEST_F(WalletLegacyApi, outcommingExternalTransactionTotalAmount) {
  class ExternalTxChecker : public Orbis::IWalletLegacyObserver {
  public:
    ExternalTxChecker(Orbis::WalletLegacy& wallet) : wallet(wallet), totalAmount(std::numeric_limits<int64_t>::max()) {
    }

    virtual void externalTransactionCreated(Orbis::TransactionId transactionId) override {
      Orbis::WalletLegacyTransaction txInfo;
  ...
  Orbis::WalletLegacy wallet(m_currency, *aliceNode);
...
namespace {
class WalletSynchronizationProgressUpdatedObserver : public Orbis::IWalletLegacyObserver {
public:
...
TEST_F(WalletLegacyApi, shutdownDoesNotRemoveObservers) {
  ...
  Orbis::WalletHelper::IWalletRemoveObserverGuard observerGuard(*alice, observer);
...
namespace {
class WalletTransactionEventCounter : public Orbis::IWalletLegacyObserver {
public:
  virtual void externalTransactionCreated(Orbis::TransactionId /*transactionId*/) override {
    ++m_count;
  }

  virtual void transactionUpdated(Orbis::TransactionId /*transactionId*/) override {
    ++m_count;
  }
...
TEST_F(WalletLegacyApi, afterShutdownAndInitWalletDoesNotSendNotificationsRelatedToOldAddress) {
  ...
  Orbis::AccountKeys accountKeys1;
  ...
  std::vector<Orbis::WalletLegacyTransfer> transfers;
...
TEST_F(WalletLegacyApi, resetDoesNotChangeAccountKeys) {
  ...
  Orbis::AccountKeys expectedAccountKeys;
  alice->getAccountKeys(expectedAccountKeys);

  alice->reset();

  Orbis::AccountKeys actualAccountKeys;
...
void generateWallet(Orbis::IWalletLegacy& wallet, TrivialWalletObserver& observer, const std::string& pass) {
...
TEST_F(WalletLegacyApi, outdatedUnconfirmedTransactionDeletedOnNewBlock) {
  ...
  Orbis::Currency currency(Orbis::CurrencyBuilder(m_logger).mempoolTxLiveTime(TRANSACTION_MEMPOOL_TIME).currency());
  TestBlockchainGenerator blockchainGenerator(currency);
  INodeTrivialRefreshStub node(blockchainGenerator);
  Orbis::WalletLegacy wallet(currency, node);
  TrivialWalletObserver walletObserver;
  wallet.addObserver(&walletObserver);
  ...
  Orbis::WalletLegacyTransaction transaction;
  ASSERT_TRUE(wallet.getTransaction(id, transaction));
  EXPECT_EQ(Orbis::WalletLegacyTransactionState::Deleted, transaction.state);
...
TEST_F(WalletLegacyApi, walletLoadsNullSpendSecretKey) {
  ...
  Crypto::generate_keys(accountKeys.address.spendPublicKey, accountKeys.spendSecretKey);
  Crypto::generate_keys(accountKeys.address.viewPublicKey, accountKeys.viewSecretKey);
  accountKeys.spendSecretKey = Orbis::NULL_SECRET_KEY;
